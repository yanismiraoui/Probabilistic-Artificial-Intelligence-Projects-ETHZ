...
class BO_algo(object):
    def __init__(self):
        ...
        # Initialize GaussiranProcessRegressor with the appropriate kernels 
        self.constraint_model = GaussianProcessRegressor(kernel= ConstantKernel(constant_value=1.5)*RBF(1.5), alpha=0.005**2) # TODO : GP model for the constraint function
        self.objective_model = GaussianProcessRegressor(kernel= ConstantKernel(constant_value=3.5)*RBF(2.0), alpha=0.01**2) # TODO : GP model for your acquisition function
    ...
    def acquisition_function(self, x: np.ndarray) -> np.ndarray:
        ...
        mu_f, sigma_f = self.objective_model.predict(x, return_std=True)
        mu_v, sigma_v = self.constraint_model.predict(x, return_std=True)

        penalty = 2

        if mu_v - 0.2 * sigma_v <= 1.2:
            return (mu_f + sigma_f * self.beta + penalty)[0]
        else:
            return (mu_f + sigma_f * self.beta)[0]
    ...
    def add_data_point(self, x: np.ndarray, z: float, c: float):
        ...
        x1 = np.atleast_2d([x[0] for x in self.previous_points])
        x2 = np.atleast_2d([x[1] for x in self.previous_points])
        y1 = np.atleast_2d([x[2] for x in self.previous_points])
        y2 = np.atleast_2d([x[3] for x in self.previous_points])

        x = np.vstack((x1,x2))

        self.objective_model.fit(np.transpose(x), np.transpose(y1))
        self.constraint_model.fit(np.transpose(x), np.transpose(y2))
    ...
    def get_solution(self) -> np.ndarray:
        ...
        """
        x_f = np.atleast_2d([x[0] for x in self.previous_points])
        x_v = np.atleast_2d([x[1] for x in self.previous_points])
        y_f = np.atleast_2d([x[2] for x in self.previous_points])
        y_v = np.atleast_2d([x[3] for x in self.previous_points])

        # When constraint is not satisfied
        y_f[y_v > 0] = 1000000

        idx = np.argmin(y_f)
        x_min = np.array([x_f[0, idx], x_v[0, idx]])
